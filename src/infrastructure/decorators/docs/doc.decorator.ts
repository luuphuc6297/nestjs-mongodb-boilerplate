...
import { ENUM_API_KEY_STATUS_CODE_ERROR } from 'constants/api-key/api-key.status-code.constant'
...
import { ENUM_ERROR_STATUS_CODE_ERROR } from 'constants/error/error.status-code.constant'
import { ENUM_FILE_EXCEL_MIME } from 'constants/file/file.enum.constant'
import { ENUM_PAGINATION_ORDER_DIRECTION_TYPE } from 'constants/pagination/pagination.enum.constant'
import { ENUM_POLICY_STATUS_CODE_ERROR } from 'constants/policy/policy.status-code.constant'
import { ENUM_REQUEST_STATUS_CODE_ERROR } from 'constants/request/request.status-code.constant'
import { ENUM_ROLE_STATUS_CODE_ERROR } from 'constants/role/role.status-code.constant'
import { FileMultipleDto } from 'infrastructure/common/file/dtos/file.multiple.dto'
import { FileSingleDto } from 'infrastructure/common/file/dtos/file.single.dto'
import {
...
export function DocAuth(options?: IDocAuthOptions) {
    ...
    if (options?.apiKey) {
        ...
        oneOfUnauthorized.push(
            ...
            {
                statusCode: ENUM_API_KEY_STATUS_CODE_ERROR.API_KEY_NOT_ACTIVE_YET_ERROR,
